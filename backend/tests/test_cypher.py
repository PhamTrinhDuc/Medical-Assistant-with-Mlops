...
def test_cypher_query_endpoint_exists(client: TestClient):
    ...
    response = client.post("/cypher/query", json={"query": "Find all patients"})
...
def test_cypher_query_response_structure(client: TestClient, mock_query_request):
    ...
    response = client.post("/cypher/query", json=mock_query_request)
...
def test_cypher_query_with_patient_search(client: TestClient):
    ...
    response = client.post(
        "/cypher/query", json={"query": "What patients have diabetes?"}
    )
...
def test_cypher_patients_endpoint(client: TestClient):
    ...
    response = client.post(
        "/cypher/patients", json={"query": "patients diagnosed in 2023"}
    )
...
def test_cypher_patients_response_structure(client: TestClient):
    ...
    response = client.post("/cypher/patients", json={"query": "Find all patients"})
...
def test_cypher_hospital_stats_endpoint(client: TestClient):
    ...
    response = client.post(
        "/cypher/hospital-stats", json={"query": "total number of patients"}
    )
...
def test_cypher_hospital_stats_response(client: TestClient):
    ...
    response = client.post(
        "/cypher/hospital-stats", json={"query": "How many visits in 2023?"}
    )
...
def test_cypher_query_empty_query(client: TestClient):
    ...
    response = client.post("/cypher/query", json={"query": ""})
...
def test_cypher_query_complex_question(client: TestClient):
    ...
    response = client.post(
        "/cypher/query",
        json={
            "query": "Which doctors have treated the most patients with hypertension in the last year?"
        },
    )
...
def test_cypher_response_format(client: TestClient):
    ...
    response = client.post("/cypher/query", json={"query": "List hospitals"})
...
def test_cypher_patients_with_condition(client: TestClient):
    ...
    response = client.post(
        "/cypher/patients", json={"query": "patients with heart disease"}
    )
...
def test_cypher_hospital_stats_query_type(client: TestClient):
    ...
    queries = ["total visits", "average visit duration", "most visited hospital"]

    for query in queries:
        response = client.post("/cypher/hospital-stats", json={"query": query})
        assert response.status_code in [200, 500]
